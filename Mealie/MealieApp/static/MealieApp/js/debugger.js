        ...
        for (let i = 0; i < food_db.length; i++)
        {
            if (food_db[i] != null)
            {
                ...
                food_item.onclick = () => {
                    ...
                    let details = document.querySelector("#food-db-" + food_db[i].pk.toString() + "-details");
                    if (details)
                    {
                        details.remove();
                    }
                    else
                    {
                        ...
                        food_details.id = "food-db-" + food_db[i].pk.toString() + "-details"; 
                        food_item.append(food_details);
                        let servings_line = document.createElement("li");
                        servings_line.innerHTML = "Serving size: " + food_db[i].fields.serving_size;
                        food_details.append(servings_line);
                        let calorie_line = document.createElement("li");
                        calorie_line.innerHTML = "Calories: " + food_db[i].fields.calories;
                        food_details.append(calorie_line);
                        let potassium_dv = document.createElement("li");
                        potassium_dv.innerHTML = "Potassium (%DV): " + food_db[i].fields.potassium_dv;
                        food_details.append(potassium_dv);
                        let calcium_dv = document.createElement("li");
                        calcium_dv.innerHTML = "Calcium (%DV): " + food_db[i].fields.calcium_dv;
                        food_details.append(calcium_dv);
                        let iron_dv = document.createElement("li");
                        iron_dv.innerHTML = "Iron (%DV): " + food_db[i].fields.iron_dv;
                        food_details.append(iron_dv);
                    }
                }
                food_item.innerHTML = food_db[i].fields.name;
